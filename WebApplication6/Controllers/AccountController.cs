...
namespace WebApplication6.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        //login validations
        public ActionResult Login(HC_USERS model )
        {
            ...
            using (LIVEDBEntities6 db = new LIVEDBEntities6())
            {
                ...
                {
                    var userd = db.HC_USERS.Where(x => x.UserName == model.UserName && x.pswd==model.Password && (x.RightsID == 9|| x.RightsID == 1||x.RightsID==12||x.RightsID==42||x.RightsID==46)).FirstOrDefault();
                    if (userd == null)
                    {
                        ViewBag.message = " wrong user name or password or dont have access for it";
                    ...
                    {
                        Session["userid"] = userd.RID;
                        var datainfo = db.hc_client_user.Where(x => x.Userid == userd.RID).FirstOrDefault();
                        if (datainfo==null)
                        {
                        ...
                        Session["username"] = userd.FirstName;
        ...
        public ActionResult ResetPassword(long id=0)
        {
            ...
            if (id == 0) {
                id = (long)Session["userid"];
            }
            using (LIVEDBEntities6 db = new LIVEDBEntities6())
            {
                var user = db.HC_USERS.Where(a => a.RID == id).FirstOrDefault();
                if (user != null)
                {
